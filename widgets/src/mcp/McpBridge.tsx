...
interface BridgeValue {
  ...
  downloadFile: (url: string, name: string, mimeType: string) => Promise<boolean>;
  notifyHeight: () => void;
}
...
export function McpBridgeProvider({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setToolData((prev) => {
        if (prev) return prev;
        import("../devMock").then((m) => setToolData(m.devOverview as ToolData));
        return prev;
      });
    }, 400);
  ...
  const callTool = useCallback(
    async (name: string, args?: Record<string, unknown>): Promise<ToolData | null> => {
      ...
      // Dev fallback: resolve from the mock module so drill-downs work in `npm run dev`.
      if (import.meta.env.DEV) {
        const m = await import("../devMock");
        return m.devResolve(name, args) as ToolData | null;
      }
  ...
  // Host-mediated file download. MCP App widgets run in a sandboxed, null-origin
  // iframe where `window.open`/anchor downloads are blocked, so we ask the host
  // to fetch + download the URL via the MCP Apps `downloadFile` (resource_link)
  // or `openLink` API. Falls back to `window.open` for dev / non-sandboxed hosts.
  const downloadFile = useCallback(
    async (url: string, name: string, mimeType: string): Promise<boolean> => {
      const a = app as unknown as Record<string, any> | null;
      if (a && isConnected) {
        try {
          if (typeof a.downloadFile === "function") {
            const r = await a.downloadFile({
              contents: [{ type: "resource_link", uri: url, name, mimeType }],
            });
        ...
        try {
          if (typeof a.openLink === "function") {
            const r = await a.openLink({ url });
            if (!r?.isError) return true;
          }
        } catch {
          /* fall through */
      ...
      // OpenAI Apps SDK host.
      try {
        const o = (window as Record<string, any>).openai;
        if (o?.openExternal) {
          await o.openExternal({ href: url });
          return true;
        }
      } catch {
        /* fall through */
      }
      // Dev / non-sandboxed last resort.
      try {
