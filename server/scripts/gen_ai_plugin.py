...
``${{MCP_ENDPOINT_URL}}`` token so the Agents Toolkit substitutes the real URL at
provision time.
...
async def main() -> None:
    ...
    # Auth block. If a Teams Developer Portal OAuth registration id is provided
    # (env OAUTH_REGISTRATION_ID, or substituted via the ${{...}} token at package
    # build time), emit OAuthPluginVault so Copilot shows a sign-in card and
    # attaches the Entra bearer token. Otherwise stay anonymous.
    ...
    if reg_id:
        ...
        # Emit the token so the package build substitutes it from env.
